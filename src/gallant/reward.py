...
class feet_stumble(Reward):
    def __init__(self, body_names, weight: float, enabled: bool = True):
        super().__init__(weight, enabled)
...
class angvel_xy_l2_torso(Reward):
    def __init__(self, weight: float, enabled: bool = True, body_names: str = None):
        super().__init__(weight, enabled)
...
class feet_contact_forces(Reward):
    def __init__(self, body_names, max_contact_force: float, weight: float, enabled: bool = True):
        super().__init__(weight, enabled)
        self.body_names = body_names
...
class feet_ground_parallel_bin(Reward): # how many points are on the ground when 
    def __init__(self, body_names, weight: float, enabled: bool = True, threshold: float = 0.035):
        super().__init__(weight, enabled)
        self.body_names = body_names
    ...
    def _initialize(self, env):
        ...
        self.body_ids_a, body_names_a = self.asset.find_bodies(self.body_names)
        self.body_ids_c, body_names_c = self.contact_forces.find_bodies(self.body_names)
...
class penalty_fast_ang_vel(Reward):
    def __init__(self, weight: float, enabled: bool = True, threshold: float = 1.0):
        super().__init__(weight, enabled)
        self.threshold = threshold
...
class contact_momentum(Reward):
    ...
    def _initialize(self, env):
        ...
        self.asset: Articulation = self.env.scene.articulations["robot"]
        self.contact_sensor: ContactSensor = self.env.scene.sensors["contact_forces"]
        self.sensor_ids, self.sensor_names = self.contact_sensor.find_bodies(self.body_names)
...
class feet_distance_lateral(Reward):
    def __init__(self, body_names, least_distance: float, most_distance: float, weight: float, enabled: bool = True):
        super().__init__(weight, enabled)
        self.body_names = body_names
...
class feet_ground_slip(Reward):
    def __init__(
        self, body_names: str, weight: float, enabled: bool = True
    ):
        super().__init__(weight, enabled)
        self.body_names = body_names
...
class waist_deviation_l2(Reward):
    def __init__(self, joint_names, weight: float, enabled: bool = True):
        super().__init__(weight, enabled)
...
class stall_penalty(Reward[LocoNavigation]):
    def __init__(self, weight: float, min_vel: float, max_vel: float, enabled: bool = True):
        super().__init__(weight, enabled)
